...
namespace Day11
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var seats = new char[width, height];
            ...
            while (true)
            {
                var nAdjacentOccupied = new int[width, height];
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        if (isOccupied)
                        {
                            ...
                            foreach (var offset in offsets)
                            {
                                var offX = offset.x + x;
                                var offY = offset.y + y;
                                while (offX >= 0 && offX < width && offY >= 0 && offY < height)
                                {
                                    if (seats[offX, offY] != '.')
                                    {
                                        nAdjacentOccupied[offX, offY]++;
                ...
                var newSeats = new char[width, height];
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        char seatResult;
                        if (seats[x, y] == '.') seatResult = '.';
                        else if (seats[x, y] == 'L' && nAdjacentOccupied[x, y] == 0)
                        {
                            changeMade = true;
                            seatResult = '#';
                        }
                        // part1
                        // else if (seats[x, y] == '#' && nAdjacentOccupied[x, y] >= 4)
                        else if (seats[x, y] == '#' && nAdjacentOccupied[x, y] >= 5)
                        {
                            changeMade = true;
                            seatResult = 'L';
                        }
                        else seatResult = seats[x, y];

                        newSeats[x, y] = seatResult;
                        
                        Console.Write(seatResult);
                    }
