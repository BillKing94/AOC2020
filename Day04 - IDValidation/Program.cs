...
namespace Day04
{
    class Program
    {
        private const string input = @"iyr:2010 ecl:gry hgt:181cm
        ...
        static void Main(string[] args)
        {
            ...
            {
                ["byr"] = (str) => int.TryParse(str, out int year) && (year >= 1920 && year <= 2002),
                ["iyr"] = (str) => int.TryParse(str, out int year) && (year >= 2010 && year <= 2020),
                ["eyr"] = (str) => int.TryParse(str, out int year) && (year >= 2020 && year <= 2030),
                ["hgt"] = (str) =>
                {
                    var match = Regex.Match(str.Trim(), "^(?<height>[\\d]+)(?<unit>cm|in)$");
                    if (match.Success)
                    {
                ...
                ["hcl"] = (str) => Regex.IsMatch(str, "^#[0-9a-f]{6}$"),
                ["ecl"] = (str) => eyeColors.Contains(str),
                ["pid"] = (str) => Regex.IsMatch(str, "^[0-9]{9}$"),
            };

            var passports = Regex.Split(input, "\n\\s*\n");

            var fieldPtn = new Regex("(?<key>.*):(?<value>\\S+)", RegexOptions.Compiled);

            int nValid = 0;

            foreach (var passport in passports)
            {
                var fields = passport.Split(new[] {"\r", "\n", " "},
                    StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

                var remainingFields = new HashSet<string>(requiredFields.Keys, StringComparer.OrdinalIgnoreCase);
                bool cidEncountered = false;
                foreach (var field in fields)
                {
                    var match = fieldPtn.Match(field);
                    var key = match.Groups["key"].Value;

                    if (remainingFields.Remove(key))
                    {
                        var value = match.Groups["value"].Value;
                        var isValid = requiredFields[key](value);
                        if (!isValid)
                        {
                            goto notvalid;
                        }
                    }
                    else if (key == "cid" && cidEncountered == false) cidEncountered = true;
                    else goto notvalid;
                }
