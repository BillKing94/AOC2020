...
namespace Day12
{
    ...
    class Program
    {
        private const string input = @"R90
                               F56
                               R90
                               F56
                               R90
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    case 'N':
                        waypointRelPos.y-=val;
                        break;
                    case 'S':
                        waypointRelPos.y += val;
                        break;
                    case 'E':
                        waypointRelPos.x += val;
                        break;
                    case 'W':
                        waypointRelPos.x -= val;
                        break;
                    case 'L':
                    {
                        while (val > 0)
                        {
                            var newX = waypointRelPos.y;
                            var newY = -waypointRelPos.x;

                            waypointRelPos.x = newX;
                            waypointRelPos.y = newY;
                            val-=90;
                        }
                    ...
                    case 'F':
                        shipPos.x += val * waypointRelPos.x;
                        shipPos.y += val * waypointRelPos.y;
                        break;
                    
                    default: throw new Exception(line.ToString());
                }

                Console.WriteLine($"ship: {shipPos}; waypoint rel: {waypointRelPos}");
            }

            var mDist = Math.Abs(shipPos.x) + Math.Abs(shipPos.y);
            Clipboard.Set(mDist);
        }
    }
}