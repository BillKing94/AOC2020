...
";
        ...
        {
            var bagContents = new Dictionary<string, (string color, int count)[]>();

            var lines = SplitNewLine(input);

            foreach (var line in lines)
            {
                var match = Regex.Match(line, "^(?<color>.*) bags contain (?<contents>.+)\\.");

                var color = match.Groups["color"].Value;
                var contents = match.Groups["contents"].Value;

                var contentsMatch = Regex.Matches(contents, "(?<count>\\d+) (?<color>[^.,]+) bag(s)?");
                var contentsColors = contentsMatch.Select(m =>
                    (color: m.Groups["color"].Value.Trim(), count: int.Parse(m.Groups["count"].Value))).ToArray();

                bagContents[color] = contentsColors;
            }

            // Debugging
            foreach (var color in bagContents.Keys)
            {
                Console.WriteLine($"{color}: {string.Join(",", bagContents[color])}");
            }

            int getNBags(string color)
            {
                var total = 1;
                foreach (var (subColor, subCount) in bagContents[color])
                {
                    var subNBags = getNBags(subColor) * subCount;
                    total += subNBags;
                }
            ...
            var result = getNBags("shiny gold");

            Console.WriteLine(result-1);

            // var containsGold = new HashSet<string>{"shiny gold"};
            //
            //
            // while (true)
            // {
            //     bool newPathFound = false;
            //     var toSearch = bagContents.Keys.ToArray();
            //     foreach (var color in toSearch)
            //     {
            //         foreach (var contentBag in bagContents[color])
            //         {
            //             if (containsGold.Contains(contentBag))
            //             {
            //                 bagContents.Remove(color);
            //                 if (containsGold.Add(color)) newPathFound = true;
            //                 break;
            //             }
            //         }
            //     }
            ...
            // Console.WriteLine(containsGold.Count - 1);
        }
    }
}