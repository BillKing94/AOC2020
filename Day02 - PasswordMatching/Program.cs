...
4-9 f: xfcfthqzw".Trim().Split("\n").ToArray();
        ...
        static void Main(string[] args)
        {
            ...
            var pattern = new Regex("(?<min>[0-9]+)-(?<max>[0-9]+) (?<ptn>.): (?<password>.*)", RegexOptions.Compiled);
            foreach(var testcase in input)
            {
                var match = pattern.Match(testcase);
                var pos1 = int.Parse(match.Groups["min"].Value);
                var pos2 = int.Parse(match.Groups["max"].Value);
                char ptn = match.Groups["ptn"].Value[0];
                var pw = match.Groups["password"].Value;

                int nMatches = 0;
                if (pw[pos1 - 1] == ptn) nMatches++;
                if (pw[pos2 - 1] == ptn) nMatches++;
