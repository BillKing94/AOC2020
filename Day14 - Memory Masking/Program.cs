...
namespace Day14
{
    ...
    class Program
    {
        private const string input = @"mask = 0XX1XXX1101X101100101001010X1X110000
                               mem[41476] = 14032
                               mem[20538] = 23975525
                               mem[44025] = 8902467
                               mem[23416] = 13278193
...
";
        ...
        private const string sample = @"mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X
                                mem[8] = 11
                                mem[7] = 101
                                mem[8] = 0";
        ...
        static void Main(string[] args)
        {
            ...
            var maskPtn = new Regex("mask = (?<mask>[X10]{36})$", RegexOptions.Compiled);
            var writePtn = new Regex(@"mem\[(?<idx>\d+)\] = (?<value>\d+)$", RegexOptions.Compiled);

            string mask = null;
            foreach (var line in lines)
            {
                var writeMatch = writePtn.Match(line);
                if (writeMatch.Success)
                {
                    var val = long.Parse(writeMatch.Groups["value"].Value);

                    var idx = long.Parse(writeMatch.Groups["idx"].Value);
                    
                    // Console.WriteLine(Convert.ToString(idx, 2).PadLeft(36, '0'));
                    // Console.WriteLine(mask);

                    for (int iBit = 0; iBit < mask.Length; iBit++)
                    {
                        var bit = mask[iBit];
                        if (bit == '1')
                        {
                            long oring = (1L << (mask.Length - 1 - iBit));
                            idx = idx | oring;
                        }
                    }

                    long nX = (long) Math.Pow(2, mask.Count(c => c == 'X'));
                    ...
                    for (int i = 0; i < nX; i++)
                    {
                        ...
                        for (int iBit = 0; iBit < mask.Length; iBit++)
                        {
                            ...
                            if (bit == 'X')
                            {
                                var bitToSet = i & (1 << nXEncountered);
                                if (bitToSet != 0)
                                {
                                    long oring = (1L << (mask.Length - 1 - iBit));
                                    idx = idx | oring;
                                ...
                                {
                                    long anding = ~(1L << (mask.Length - 1 - iBit));
                                    idx = idx & anding;
                                }
                    ...
                    // part1
                    // Console.WriteLine(Convert.ToString(val, 2).PadLeft(36, '0'));
                    // Console.WriteLine(mask);
                    // for (int iBit = 0; iBit < mask.Length; iBit++)
                    // {
                    //     var bit = mask[iBit];
                    ...
                    // Console.WriteLine();
                    //
                    // var idx = int.Parse(writeMatch.Groups["idx"].Value);
                    // memory[idx] = val;
                }
                else
                {
                    var maskMatch = maskPtn.Match(line);
